...
void cal_complex(float num1_real, float num1_image, float num2_real, float num2_image) {
    ...
    float test = fp16_to_fp32(num1_real_fp16);
    uint32_t bits;
    memcpy(&bits, &test, sizeof(test));
    printf("num1_real_fp16: %x \n", bits);

    test = fp16_to_fp32(num1_image_fp16);
    memcpy(&bits, &test, sizeof(test));
    printf("num1_image_fp16: %x \n", bits);

    test = fp16_to_fp32(num2_real_fp16);
    memcpy(&bits, &test, sizeof(test));
    printf("num2_real_fp16: %x \n", bits);

    test = fp16_to_fp32(num2_image_fp16);
    memcpy(&bits, &test, sizeof(test));
    printf("num2_image_fp16: %x \n", bits);
